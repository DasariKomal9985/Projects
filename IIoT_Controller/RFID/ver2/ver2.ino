...
void loop() {
  ...
  if (isValid) {
    ...
    drawStatusBox(20, 130, true);   // left tick
    ...
    drawStatusBox(20, 130, false);   // left cross
...
void drawGateClosed() {
  ...
  tft.fillRect(10, 120, 40, 80, ST77XX_WHITE);              // Left terminal
  tft.fillTriangle(50, 120, 70, 120, 50, 200, ST77XX_RED);  // Flap
  drawStatusBox(20, 130, false);
}

void drawGateDenied() {
  drawStatusBox(20, 130, false);  // Cross only
}
...
void drawGateOpenAnimated() {
  ...
  drawStatusBox(20, 130, true);  // Final tick mark
}
...
void drawGateCloseAnimated() {
  ...
  drawStatusBox(20, 130, false);  // Cross again
}

void drawGateStep(int offset) {
  // Clear previous flap area
  tft.fillTriangle(50, 120, 70, 120, 50, 200, ST77XX_BLACK);  // Left terminal

  // Triangle vertices for right-angled triangle (left gate)
  int topLeftX = 50;
  int topLeftY = 120;
  int topRightX = 70 - offset;
  int topRightY = 120;
  int bottomX = 50;
  int bottomY = 200;
...
void drawGateClosedRight() {
  tft.fillRect(160, 120, 80, 80, ST77XX_BLACK);                // Clear right block only
...
void drawGateStepRight(int offset) {
  // Clear previous flap area
  tft.fillTriangle(275, 150, 260, 150, 280, 230, ST77XX_BLACK);  // Right terminal
